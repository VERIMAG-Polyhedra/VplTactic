...
let find_constant contrib dir s =
  lazy (Universes.constr_of_global (Coqlib.find_reference contrib dir s))
...
let mkLambda (t:Term.types) (b:Term.constr): Term.constr =
  Term.mkLambda (Names.Name.Anonymous, t, b)
...
module Positive = struct
  ...
  let reify_as_RatZ =
    let rec reify_with_shift (s:int) acc p =
      if Term.eq_constr p (Lazy.force xH) then
        Rat.Z.orL acc (Rat.Z.shiftL Rat.Z.u s)
      else 
        match decomp_term p with
        | Term.App(head, args) when Term.eq_constr head (Lazy.force _xO) ->
           reify_with_shift (s+1) acc args.(0)
        | Term.App(head, args) when Term.eq_constr head (Lazy.force _xI) ->
...
struct
  ...
  let reify_as_RatZ p =
    if Term.eq_constr p (Lazy.force _Z0) then
      Rat.Z.z
    else 
      match decomp_term p with
      | Term.App(head, args) when Term.eq_constr head (Lazy.force _Zpos) ->
         Positive.reify_as_RatZ args.(0)
      | Term.App(head, args) when Term.eq_constr head (Lazy.force _Zneg) ->
         Rat.Z.neg (Positive.reify_as_RatZ args.(0))
      | _ -> raise RatReifyError
...
module Qc = struct
  ...
  let reify_as_Rat p =
    match decomp_term p with
    | Term.App(head, args) when Term.eq_constr head (Lazy.force _Qcmake) -> (
       match decomp_term args.(0) with
       | Term.App(head, args) when Term.eq_constr head (Lazy.force _Qmake) ->
          let num = Z.reify_as_RatZ args.(0) in
          let den = Positive.reify_as_RatZ args.(1) in
          Rat.ofZ num den
       | _ -> raise RatReifyError)
    | _ -> raise RatReifyError
...
module Input = struct
  ...
  type varmap = Term.constr
  type term = Term.constr
  ...
  let reify_as_cmpT t = 
    if Term.eq_constr t (Lazy.force _LeT) then
      Cstr.Le
    else if Term.eq_constr t (Lazy.force _EqT) then
      Cstr.Eq
    else if Term.eq_constr t (Lazy.force _LtT) then
      Cstr.Lt
    else
      assert false
...
module Varmap = struct
  ...
  type key = int * AffTerm.var

  type state = (Term.constr * key) list
  ...
  let add (env : t) (t : Term.constr ) =
    try  snd (ConstrHashtbl.find env.map t) 
    with
    | Not_found -> (
       let n = env.nxt in
       ConstrHashtbl.add env.map t n;
       let p = (fst n)+1 in
       env.undo_stack <- (t, env.nxt)::env.undo_stack;
       env.nxt <- (p, to_path (p+1));
       snd n)
  ...
  let rec restore env l =
    ...
    if env.undo_stack != l then
      ...
      | (t, nxt)::stack -> (
        ConstrHashtbl.remove env.map t;
  ...
  let empty () =
    ...
    { map = ConstrHashtbl.create 16;
      undo_stack = [];
      nxt = (0, (lapp Input._Var [|xH|], { AffTerm.lin = Vec.mk [(Rat.u,Var.XH)]; AffTerm.cst = Rat.z })) }

  let to_varmap env =
    let n = fst env.nxt in
    ...
    let rec build p =
      if p > n then
        ...
        let p0 = p lsl 1 in
        Term.mkApp (node_vm, [| a.(p-1); build p0; build (p0 lor 1) |])
      )
    in
      ConstrHashtbl.iter (fun constr k -> a.(fst k) <- constr) env.map;
      build 1
...
module List = struct
  ...
  let map ty f l =
    ...
    let rec map_acc l acc =
      match decomp_term l with
      | Term.App(head, args) when Term.eq_constr head cons ->
         map_acc args.(2) (Term.mkApp (cons, [| ty; f args.(1); acc|]))
      | _ ->
         acc
    in map_acc l (nil ty)
...
module Prod = struct
  ...
  let set_fst ty f p =
    ...
    match decomp_term p with
    | Term.App(head, args) when Term.eq_constr head pair ->
      Term.mkApp (pair, [| ty; args.(1); (f args.(2) args.(3)); args.(3) |])
...
struct
  ...
  type fexp = Term.constr
  type v = Term.constr
  type list_fexp =Term.constr * Term.constr * Term.constr * Term.constr
  type pedra = Term.constr
  type script = Term.constr
  type proof = Term.constr
  let mkv x = Term.mkRel x
